...
/**
 * Shared data types for mega menus
 */
...
export interface NavMegaMenuProps {
  categories: Category[];
  content: Record<string, ContentColumn[]>;
...
/**
 * NavMegaMenu - Shared mega menu component for Products, SaaS, Technology, etc.
 * Features:
 * - Configurable categories in sidebar
 * - 3-column layout for menu items
 * - Smooth category switching animation
 * - Reusable across multiple menu types
 */
export function NavMegaMenu({ categories, content, defaultCategory = "" }: NavMegaMenuProps) {
  const initialCategory = defaultCategory || categories[0]?.id || "";
  const [activeCategory, setActiveCategory] = useState<string>(initialCategory);

  const activeContent = content[activeCategory] || content[initialCategory] || [];
